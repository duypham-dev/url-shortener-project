...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true);
      const response: any = await axiosClient.post('/auth/register', { email, username, password });
    ...
    <div className="flex min-h-screen bg-white font-sans text-slate-900">
      ...
      <div className="flex-1 flex flex-col px-6 py-8 md:px-12 lg:px-24 xl:px-32 relative overflow-y-auto">
        ...
        <div className="w-full max-w-[420px] mx-auto flex-1 flex flex-col justify-center pb-20">
          <h1 className="text-[32px] font-extrabold mb-2 text-slate-900 tracking-tight">Create your account</h1>
          <p className="text-slate-600 mb-10 text-[15px]">
            Already have an account?{' '}
            <Link to="/login" className="text-[#3452cf] hover:underline font-medium">Log in</Link>
          </p>
          ...
          <button
            type="button"
            className="w-full bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold py-3.5 px-4 rounded transition-colors flex justify-center items-center gap-3 shadow-sm"
          >
